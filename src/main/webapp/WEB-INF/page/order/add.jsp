...
<!-- PAGE CONTENT BEGINS -->
    ...
    <form class="form-horizontal" role="form" id="form1" action="${ctxPath}/order/save.action" method="post">
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-9">
            <div class="clearfix">
                <input type="text" name="customerDiscount" id="customerDiscount" class="col-xs-12 col-sm-6" readonly value="${order.discount}" />%
            </div>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-9">
            <div class="clearfix">
                <input type="text" name="coachUsername" id="coachUsername" class="col-xs-12 col-sm-6" value="${order.coachUsername}" />
            </div>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-9">
            <div class="clearfix">
                <input type="text" name="price" id="price" class="col-xs-12 col-sm-6" readonly value="${order.price}" />
            </div>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-9">
            <div class="clearfix">
                <input type="text" name="proportion" id="proportion" class="col-xs-12 col-sm-6" readonly value="${order.proportion}"/>%
            </div>
        </div>
    </div>

    <div class="form-group">
        <label class="control-label col-xs-12 col-sm-3 no-padding-right" for="coachSalary">陪练收益</label>
...
<script type="text/javascript">
    ...
    jQuery(function ($) {
        ...
        var refreshBalance = function() {
            ...
            var balance = (parseFloat(price) * diffHours).toFixed(2);
            var payment = (balance * (customerDiscount / 100)).toFixed(2);
            var coachSalary = (parseFloat(proportion) / 100 * payment).toFixed(2);
        ...
        var changeCoachUsername = function() {
            var coachUsername = $("#coachUsername").val();
            if(!coachUsername) return;
            $.getJSON("${ctxPath}/ajax/user/get_user.action", {"username" : coachUsername, "userType" : 'COACH'}, function(data) {
                $("#proportion").val("");
                $("#price").val("");
                coachInputSuccess = false;
                if(!data || !data.id) {
                    bootbox.alert("输入的陪练用户名在系统不存在");
                    return;
                }
                coachInputSuccess = true;
        ...
        $("#coachUsername").change(changeCoachUsername);
        ...
        $('#form1').validate({
            ...
            submitHandler: function (form) {
                ...
                if(!coachInputSuccess) {
                    bootbox.alert("请输入正确的陪练用户名");
                    return false;
                }
                form.submit();
            },
